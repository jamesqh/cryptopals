...
from challenges.common_functions import (sha1, DSAParams,
                                         recover_dsa_key_from_k, modinv)
...
if __name__ == "__main__":
    try:
        ...
        if r.status_code != 200:
            raise RuntimeError("Can't find data file 44.txt "
    ...
    for match in matches:
        msg, s, r, m = match
        signatures.append({"msg": msg, "s": int(s),
                           "r": int(r), "m": int(m, 16)})
    ...
    for sig1, sig2 in attack_pairs:
        ...
        key_guess_1 = recover_dsa_key_from_k(params, sig1["msg"],
                                             (sig1["r"], s1), k)
        key_guess_2 = recover_dsa_key_from_k(params, sig2["msg"],
                                             (sig2["r"], s2), k)
    ...
    assert (sha1(hex(key_guesses[0])[2:]).hex()
